...
trap_init(void)
{
	...
	// THE PRIVILEGE LEVEL IS ONLY FOR "int $..." INSTRUCTION
	// IF IT IS REALLY GENERATED BY HARDWARE, THIS ITEM IS IGNORED
	// for example, if divide fault is generated by hardware when 
	// cpu tries to compute 1/0, then it will always cause a divide
	// fault, even if DPL = 0 and CPL = 3;
...
trap_dispatch(struct Trapframe *tf)
{
	...
	switch (tf->tf_trapno){
// divide fault is really a fault...
// if you don't change the divisor, and resume the process, the divide 
// instruction will be executed again...so again and again...
//	case T_DIVIDE:
//		cprintf("divide zero catched\n");
//		return;
	case T_PGFLT:
		if ( (tf->tf_cs & 3) == 0 ){
			print_trapframe(tf);
