...
pgfault(struct UTrapframe *utf)
{
	...
	uint32_t pentry = *(uint32_t*)PGADDR(PDX(UVPT), PDX(addr), PTX(addr) * 4);
...
duppage(envid_t envid, unsigned pn)
{
	...
	uint32_t pentry = *(uint32_t*)PGADDR(PDX(UVPT), PDX(pn*PGSIZE), PTX(pn*PGSIZE) * 4);
	if ( !(pentry & PTE_P) ) return 0;
	assert( pentry & PTE_U );

	if ( (pentry & PTE_W) || (pentry & PTE_COW) ){
...
fork(void)
{
	...
	if ( childid > 0 ){
		...
		for ( uint32_t pn = USTABDATA/PGSIZE; pn < USTACKTOP/PGSIZE;){
			if ( !(*(uint32_t*)PGADDR(PDX(UVPT), PDX(UVPT), PDX(pn*PGSIZE) * 4) & PTE_P) ){
